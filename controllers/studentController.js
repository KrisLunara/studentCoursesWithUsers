...
module.exports.viewProfile = async function (req, res) {
    ...
    //if not staff nor student, redirect
...
module.exports.renderEditForm = async function (req, res) {
    const isAdmin = req.user.can('edit self') && req.user.matchesStudentId(req.params.id);
    const profileBelongsToUser = req.user.can('edit self') && req.user.matchesStudentId(req.params.id);

    //if not staff but student, redirect
    if (!isAdmin && !profileBelongsToUser){
        res.redirect('/')
        return
    }
...
module.exports.enrollStudent = async function (req, res) {
    ...
    //if not staff nor student's profile, redirect
    if (!isAdmin && !profileBelongsToUser){
        res.redirect('/')
        return
    }
